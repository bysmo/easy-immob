        ...
        <div class="flex-1 flex flex-col min-w-0 transition-all duration-200 ease-in-out"
            ...
            <header class="sticky top-0 z-30 h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 flex items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <!-- Agency / Landlord / Tenant Badge -->
                    @if(auth()->user()?->isOwner())
                        <div class="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-xs font-semibold text-emerald-800 dark:text-emerald-300 border border-emerald-200/80 dark:border-emerald-800">
                            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span>{{ auth()->user()->owner?->full_name ?? auth()->user()->name }}</span>
                        </div>
                    @elseif(auth()->user()?->isTenant())
                        <div class="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-xs font-semibold text-emerald-800 dark:text-emerald-300 border border-emerald-200/80 dark:border-emerald-800">
                            <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span>{{ auth()->user()->tenant?->full_name ?? auth()->user()->name }}</span>
                        </div>
