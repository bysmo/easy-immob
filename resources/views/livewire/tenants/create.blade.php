<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 dark:border-slate-800 pb-4">
        <div>
            ...
            <h1 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Nouveau locataire</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400">Renseignez la fiche d'identité et les coordonnées de l'occupant.</p>
        </div>
    </div>
        ...
        <x-card>
            <div class="flex items-center gap-3 pb-4 mb-5 border-b border-slate-100 dark:border-slate-800">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500">Nom et prénom figurant sur la pièce d'identité.</p>
                </div>
            </div>
            ...
            <div class="space-y-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                        ...
                        <x-input wire:model="email" type="email" id="email" placeholder="marc.kouassi@example.com" icon="notifications" :error="$errors->first('email')" />
                    </div>

                    <div>
                        <x-label for="phone">Numéro de Téléphone</x-label>
                        <x-input wire:model="phone" type="text" id="phone" placeholder="+225 05 00 00 00 00" :error="$errors->first('phone')" />
                    </div>
                </div>

                <div>
                    <x-label for="address">Adresse de résidence actuelle</x-label>
                    <x-input wire:model="address" type="text" id="address" placeholder="Ex: Marcory Zone 4, Abidjan" :error="$errors->first('address')" />
                </div>

                <div>
                    <x-label for="emergency_contact">Contact en cas d'urgence / Personne à prévenir</x-label>
                    <x-input wire:model="emergency_contact" type="text" id="emergency_contact" placeholder="Ex: Kouassi Paul (Frère) - +225 01 00 00 00 00" icon="user" :error="$errors->first('emergency_contact')" />
                </div>
            </div>
        </x-card>
