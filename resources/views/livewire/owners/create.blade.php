<div class="max-w-2xl space-y-5">
    <div class="flex items-center gap-3">
        <a href="{{ route('owners.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400">← Retour</a>
        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Nouveau propriétaire</h1>
    </div>
        ...
        <form wire:submit="save" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-input wire:model="first_name" type="text" id="first_name" placeholder="Jean" autofocus />
                    @error('first_name') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <x-label for="last_name">Nom</x-label>
                    <x-input wire:model="last_name" type="text" id="last_name" placeholder="Dupont" />
                    @error('last_name') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
            </div>

            <div>
                <x-label for="company_name">Raison sociale (facultatif si personne morale)</x-label>
                <x-input wire:model="company_name" type="text" id="company_name" placeholder="SCI Immobilière" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <x-input wire:model="email" type="email" id="email" placeholder="jean.dupont@example.com" />
                    @error('email') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>

                <div>
                    <x-label for="phone">Téléphone</x-label>
                    <x-input wire:model="phone" type="text" id="phone" placeholder="+225 07 00 00 00 00" />
                    @error('phone') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                </div>
            </div>

            <div>
                <x-label for="address">Adresse physique</x-label>
                <x-input wire:model="address" type="text" id="address" placeholder="Cocody Angré, Abidjan" />
